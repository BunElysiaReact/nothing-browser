...
bool piggy_handleInteraction(PiggyServer *srv, const QString &c,
    ...
    // ── refresh — hard reload of the current tab, browser-refresh-button style ─
    if (c == "refresh") {
        p->triggerAction(QWebEnginePage::ReloadAndBypassCache);
        srv->respond(client, id, true, "refreshed");
        return true;
    }
