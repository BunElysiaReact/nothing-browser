...
QString piggy_createTab(PiggyServer *srv) {
    ...
    // PiggyPage (not plain QWebEnginePage) — needed so dialog.*/upload
    // commands can intercept javaScriptAlert/Confirm/Prompt and chooseFiles.
    auto *p = new PiggyPage(profile, srv);
    ...
    // navigate/exposed_call above — lets JS-side onDialog()/onCaptcha()-style
    // listeners react even outside a manual dialog.accept/dismiss flow.
    QObject::connect(p, &PiggyPage::dialogRequested, srv,
        [srv, id](const QString &dialogType, const QString &message) {
            QJsonObject event;
            event["type"]        = "event";
            event["event"]       = "dialog";
            event["tabId"]       = id;
