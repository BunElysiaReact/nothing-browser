...
class PiggyClient extends EventEmitter {
  ...
  async probe() {
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        // Wait briefly then close - just checking if we can connect
        setTimeout(() => {
          ws.close();
          resolve(true);
        }, 100);
      ...
      ws.on('close', () => {
        // Only resolve false if we never got open
        if (ws.readyState === WebSocket.CLOSED && !this.connected) {
          // Don't resolve here since open/error already did
        }
      });
  ...
  async connect() {
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('open', () => {
        // Don't resolve yet - wait for the 'ready' message from server
        logger.debug('WebSocket opened, waiting for ready signal...');
      });
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          // Handle the 'ready' message sent by server on successful connection
          if (msg.type === 'ready') {
